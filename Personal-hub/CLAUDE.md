...
- **Version number** — currently v4.1. Shown in sidebar footer, mobile drawer footer, About page badge, and `sw.js` cache name. **Every commit that changes code MUST bump the version** — no exceptions. Bump minor version each time (v3.5 → v4.1 → v4.1 etc). **Major structural shifts** bump the major version (v3.x → v4.1). The 5 locations to update on every bump: (1) sidebar footer in `index.html`, (2) mobile drawer footer in `index.html`, (3) About page badge in `index.html`, (4) `sw.js` cache name (`baker-hub-vX.Y`), (5) this line in CLAUDE.md.
- **Service worker caching** — cache name must match version (currently `baker-hub-v4.1`). Bump after significant changes or users see old cached pages.
